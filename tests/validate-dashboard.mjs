...
assert(html.includes('<th>Action</th>'), 'portfolio table must include an action column for deleting custom stocks');
assert(html.includes('Professional roadmap'), 'settings page should include professionalization recommendations');

const app = await readFile('src/app.js', 'utf8');
for (const feature of ['scoreAsset', 'refreshCryptoPrices', 'parsePortfolioCsv', 'renderRecommendations', 'requireDashboardPassword', 'technicalChart', 'analyzeAsset', 'data-delete-position']) {
...
assert(app.includes('Support / resistance'), 'asset detail should expose support/resistance chart analysis');
assert(app.includes('Risk/reward'), 'asset detail should expose risk/reward analysis');
